...
public class TriagemConsumerListenerImpl implements TriagemConsumerListener {
    ...
    @Override
    public void listener(ConsumerRecord<String, Ficha> record) {
        System.out.println(record.value().getToken().getNumToken());
        try {

        guicheService.adicionarFila(record.value().getToken());
        }catch (Exception e){
